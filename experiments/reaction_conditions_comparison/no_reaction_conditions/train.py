#!/usr/bin/env python3
"""
Train model without reaction condition features.
This model excludes: temperature, polytype_emb_1, polytype_emb_2, 
method_emb_1, method_emb_2, solvent_logP, solvent_TPSA, solvent_HBD, 
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train model without reaction conditions")
...
def load_presplit_data(config):
    ...
    # Get available features and remove reaction condition features
    all_features = [c for c in prediction_utils.feature_columns if c in df_train.columns]
    
    # Features to exclude (reaction conditions)
    ...
    # Filter out reaction condition features
    available_features = [f for f in all_features if f not in reaction_condition_features]
    
    print(f"\nUsing {len(available_features)} features (without reaction conditions)")
    print(f"Excluded {len(reaction_condition_features)} reaction condition features:")
    for feat in reaction_condition_features:
        if feat in all_features:
            print(f"  - {feat}")
    
    print(f"\n  Train: {len(df_train)} samples ({df_train['reaction_id'].nunique()} groups)")
...
def evaluate_and_save(model_info, df_holdout, config):
    ...
    metadata = {
        'experiment': 'no_reaction_conditions',
...
def main():
    ...
    print("="*60)
    print("NO REACTION CONDITIONS EXPERIMENT")
    print("="*60)
