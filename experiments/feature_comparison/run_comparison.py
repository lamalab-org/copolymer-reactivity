...
def train_xgboost(df_train, features, config, training_config=None):
    ...
    if training_config and training_config.get('remove_specialized', False):
        if 'specialized_filter' in df_train_work.columns:
            ...
            df_train_work = df_train_work[df_train_work['specialized_filter'] != 'specialized'].reset_index(drop=True)
...
# Plotting
# ---------------------------------------------------------------------------
def create_comparison_plot(res_quantum, res_morgan, n_feats_quantum,
                           n_feats_morgan, output_dir):
    ...
    fig, (ax1, ax2, ax3) = plt.subplots(
        1, 3, figsize=(TWO_COL_WIDTH_INCH, height)
    )
    ...
    b1 = ax1.bar(x - w / 2, q_vals, w, label='Quantum Descriptors',
                 color=c1, alpha=alpha)
    b2 = ax1.bar(x + w / 2, m_vals, w, label='Morgan Fingerprints',
    ...
    classes = ['Class 0\n(Alternating)', 'Class 1\n(Block-like)',
               'Class 2\n(Homopolymer)']
    x2 = np.arange(3)
    ax2.bar(x2 - w / 2, res_quantum['per_class_acc'], w,
            color=c1, alpha=alpha)
    ax2.bar(x2 + w / 2, res_morgan['per_class_acc'], w,
            color=c2, alpha=alpha)
    for vals, offset in [(res_quantum['per_class_acc'], -w / 2),
                         (res_morgan['per_class_acc'], w / 2)]:
        for i, v in enumerate(vals):
            ax2.text(i + offset, v + 0.01, f'{v:.2f}',
    ...
    for i, cnt in enumerate(counts):
        ax3.text(i, cnt * 1.15, str(int(cnt)),
...
def main():
    ...
    if args.plot_only:
        ...
        create_comparison_plot(res_quantum, res_morgan,
                               n_feats_q, n_feats_m, output_dir)
        print("\nDone.")
        return
    ...
    try:
        ...
        print(f"    - Specialized removed: {training_config.get('specialized_removed_from_training', False)}")
        ...
        quantum_features = [c for c in prediction_utils.feature_columns
                            if c in df_train.columns]
    print(f"  Quantum features: {len(quantum_features)}")

    # Lookup pool: train + negative data (only if final model uses negative data)
    use_negative_in_lookup = training_config.get('negative_data_used', False) if training_config else False
    ...
    if args.use_test:
        ...
        df_eval_morgan = df_test_morgan[df_test_morgan['reaction_id'].isin(df_eval['reaction_id'])].reset_index(drop=True)
        ...
        df_eval_morgan = df_test_morgan[df_test_morgan['reaction_id'].astype(str).isin(val_reaction_ids)].reset_index(drop=True)
        print(f"  ⚠️  Note: Using test_morgan.csv filtered to validation reaction_ids")
        print(f"     Morgan validation subset: {len(df_eval_morgan)} samples (of {len(df_test_morgan)} total)")
        if len(df_eval_morgan) != len(df_eval):
            print(f"     ⚠️  Warning: Sample count mismatch! Validation: {len(df_eval)}, Morgan: {len(df_eval_morgan)}")
            print(f"        Some validation samples may not be in Morgan test set")
    morgan_features = [c for c in get_morgan_feature_columns(config['n_bits'])
                       if c in df_train_morgan.columns]
    ...
    lookup_pred, _ = compute_naive_baseline_predictions_with_similarity(
        df_eval, df_lookup_pool, y_lookup_pool, quantum_features,
        fp_dict=fp_dict,
    )
    ...
    df_eval_morgan_matched['_sort_key'] = df_eval_morgan_matched['reaction_id'].astype(str).apply(
        lambda x: list(eval_reaction_ids).index(x) if x in eval_reaction_ids else 999999
    )
    ...
    if len(df_eval_morgan_matched) != len(df_eval):
        print(f"  ⚠️  Warning: Only {len(df_eval_morgan_matched)}/{len(df_eval)} samples matched in Morgan data")
        print(f"     Some samples will be excluded from comparison")
        # Filter df_eval and other arrays to matched samples
        matched_mask = df_eval['reaction_id'].astype(str).isin(df_eval_morgan_matched['reaction_id'].astype(str))
    ...
    res_quantum = evaluate_voting(xgb_pred_quantum, lookup_pred, y_true,
                                  "Quantum Descriptors")
    res_morgan = evaluate_voting(xgb_pred_morgan, lookup_pred, y_true,
                                 "Morgan Fingerprints")
    ...
    print(f"{'CV score (XGBoost)':<25} {cv_quantum:>12.4f} {cv_morgan:>12.4f} "
          f"{cv_morgan - cv_quantum:>+10.4f}")
    print(f"{'Num features (XGBoost)':<25} {len(quantum_features):>12d} "
    ...
    create_comparison_plot(res_quantum, res_morgan,
                           len(quantum_features), len(morgan_features),
                           output_dir)
