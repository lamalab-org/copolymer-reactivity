#!/usr/bin/env python3
"""
Train baseline model with current features (quantum chemical descriptors).
"""
...
def load_presplit_data(config):
    ...
    if not os.path.exists(train_path) or not os.path.exists(test_path):
        ...
        print(f"  - {train_path}")
        print(f"  - {test_path}")
        print(f"\nRun: python create_train_test_split.py")
        sys.exit(1)
    ...
    # Note: NaN removal already done in central split creation
