...
def calculate_shap_importance_by_groups(
    ...
    reduction='mean', group_labels=None,
):
    """
    ...
    if isinstance(shap_values, list):
        ...
        print(f"  Multi-class (list): {len(shap_values)} classes, SHAP shape after {reduction}: {shap_abs.shape}")
    elif len(shap_values.shape) == 3:
        # 3D array: (n_samples, n_features, n_classes)
        shap_abs = reduce_fn(np.abs(shap_values), axis=2)  # (n_samples, n_features)
        print(f"  Multi-class (3D array): SHAP shape {shap_values.shape} -> {shap_abs.shape} after {reduction} over classes")
    else:
...
def build_named_groups_for_paper(feature_names: list[str]) -> dict[str, list[str]]:
    ...
    groups["Fukui index (electrophilicity) max"] = _take(["fukui_electrophilicity_max_1", "fukui_electrophilicity_max_2"])
    ...
    groups["Fukui index (nucleophilicity) max"] = _take(["fukui_nucleophilicity_max_1", "fukui_nucleophilicity_max_2"])
    groups["Ionization potential (corrected)"] = _take(["ip_corrected_1", "ip_corrected_2"])
    groups["Fukui index (nucleophilicity) mean"] = _take(["fukui_nucleophilicity_mean_1", "fukui_nucleophilicity_mean_2"])
    groups["Global nucleophilicity"] = _take(["global_nucleophilicity_1", "global_nucleophilicity_2"])
    groups["Global electrophilicity"] = _take(["global_electrophilicity_1", "global_electrophilicity_2"])
    groups["LUMO"] = _take(["lumo_1", "lumo_2"])
    groups["Fukui index (radical) mean"] = _take(["fukui_radical_mean_1", "fukui_radical_mean_2"])
    groups["Fukui index (nucleophilicity) min"] = _take(["fukui_nucleophilicity_min_1", "fukui_nucleophilicity_min_2"])
    groups["Solvent number of hydrogen bond donors"] = _take(["solvent_HBD"])
    groups["Fukui index (electrophilicity) mean"] = _take(["fukui_electrophilicity_mean_1", "fukui_electrophilicity_mean_2"])
    groups["Solvent Topological Polar Surface Area"] = _take(["solvent_TPSA"])
    groups["Electron affinity"] = _take(["ea_1", "ea_2"])
    groups["Fukui index (electrophilicity) min"] = _take(["fukui_electrophilicity_min_1", "fukui_electrophilicity_min_2"])
...
def aggregate_per_feature_importance_to_groups(
    ...
    *,
    ...
    if per_feature_df is None or len(per_feature_df) == 0:
        return pd.DataFrame(columns=["group_label", "features", "n_features", "importance_mean", "importance_std"])
...
def build_pair12_with_correlation_groups(
    ...
    *,
    ...
    available_labels = [lbl for lbl, feats in atomic.items()
                        if all(f in X_df.columns for f in feats)]
