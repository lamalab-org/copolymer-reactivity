...
def filter_data(data):
    ...
    """
    filtered_data = [entry for entry in data if entry['r-product_filter'] and entry['r_conf_filter']]
    ...
    for monomer_key in ['monomer1_data', 'monomer2_data']:
        ...
        for key in keys_to_process:
            df[f"{monomer_key}_{key}_min"] = df[monomer_key].apply(
                lambda x: min(x[key].values()) if isinstance(x, dict) and key in x else None
            )
            df[f"{monomer_key}_{key}_max"] = df[monomer_key].apply(
                lambda x: max(x[key].values()) if isinstance(x, dict) and key in x else None
            )
            df[f"{monomer_key}_{key}_mean"] = df[monomer_key].apply(
                lambda x: sum(x[key].values()) / len(x[key].values()) if isinstance(x, dict) and key in x else None
            )

        # Process dipole components
        df[f"{monomer_key}_dipole_x"] = df[monomer_key].apply(
            lambda x: x['dipole'][0] if isinstance(x, dict) and 'dipole' in x and isinstance(x['dipole'], list) else None
        )
        df[f"{monomer_key}_dipole_y"] = df[monomer_key].apply(
            lambda x: x['dipole'][1] if isinstance(x, dict) and 'dipole' in x and isinstance(x['dipole'], list) else None
        )
        df[f"{monomer_key}_dipole_z"] = df[monomer_key].apply(
            lambda x: x['dipole'][2] if isinstance(x, dict) and 'dipole' in x and isinstance(x['dipole'], list) else None
        )
...
def create_flipped_dataset(df_features):
    ...
    for _, row in df_features.iterrows():
        ...
        flipped_row['monomer1_s'], flipped_row['monomer2_s'] = row['monomer2_s'], row['monomer1_s']
...
def main(input_file, output_file):
    ...
    df = df.dropna(
        subset=['r1', 'r2', 'monomer1_s', 'monomer2_s', 'temperature', 'determination_method', 'polymerization_type']
    )
    ...
    print(f"Processed data saved to {output_file} and flipped data to {output_file.replace('.csv', '_flipped.csv')}.")
    print(f"Number of datapoints in {output_file}: {len(df)}")
    print(f"Number of datapoints in {output_file.replace('.csv', '_flipped.csv')}: {len(df_flipped)}")
