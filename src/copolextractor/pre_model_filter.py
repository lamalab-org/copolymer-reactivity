...
def preprocess_data(data):
    for entry in data:
        if 'r_values' in entry:
            entry['r_values']['constant_1'] = convert_to_float(entry['r_values'].get('constant_1'))
            entry['r_values']['constant_2'] = convert_to_float(entry['r_values'].get('constant_2'))
        if 'conf_intervals' in entry:
            entry['conf_intervals']['constant_conf_1'] = convert_to_float(entry['conf_intervals'].get('constant_conf_1'))
            entry['conf_intervals']['constant_conf_2'] = convert_to_float(entry['conf_intervals'].get('constant_conf_2'))
...
if __name__ == "__main__":
    ...
    output_file = "../../copol_prediction/output/extracted_data_w_features_filtered.json"
