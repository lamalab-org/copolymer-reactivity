...
class XTB(OriginalXTB):
    ...
    def _parse_json(self, json_file: Path) -> None:
        ...
        if "fukui indices" in data:
            ...
            if "f(+)" in fukui_data:
                self._results.fukui_plus = {i: val for i, val in enumerate(fukui_data["f(+)"])}
            # Extract Fukui- (nucleophilic)
            if "f(-)" in fukui_data:
                self._results.fukui_minus = {i: val for i, val in enumerate(fukui_data["f(-)"])}
            # Extract Fukui0 (radical)
            if "f(0)" in fukui_data:
                self._results.fukui_radical = {i: val for i, val in enumerate(fukui_data["f(0)"])}
