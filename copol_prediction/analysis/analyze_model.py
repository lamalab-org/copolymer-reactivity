...
def load_train_with_negative_data(split_dir):
    ...
    neg_path = os.path.join(project_root, "filter", "artificial_datapoints",
    ...
    if os.path.exists(neg_path):
        ...
        print(f"  ✓ Added {len(df_neg)} negative data points to lookup pool ({len(df_train)} total)")
    else:
...
def compute_naive_baseline_predictions(df_test, df_train, y_train, feature_cols=None, fp_dict=None):
    ...
    if fp_dict is None:
        ...
        unique_monomer1 = set(df_test['monomer1_smiles'].dropna().unique()) | set(df_train['monomer1_smiles'].dropna().unique())
        unique_monomer2 = set(df_test['monomer2_smiles'].dropna().unique()) | set(df_train['monomer2_smiles'].dropna().unique())
        unique_solvents = set(df_test['solvent_smiles'].dropna().unique()) | set(df_train['solvent_smiles'].dropna().unique())
        ...
        print(f"  Computed fingerprints for {len([v for v in fp_dict.values() if v is not None])} valid SMILES")
    ...
    for idx, (mon1, mon2, solv) in enumerate(zip(
        df_train['monomer1_smiles'], 
        df_train['monomer2_smiles'], 
        df_train['solvent_smiles']
    )):
    ...
    for test_pos, (test_idx, test_row) in enumerate(df_test.iterrows()):
        ...
        if test_mon1_fp is None or test_mon2_fp is None or test_solv_fp is None:
            ...
            mon1_flipped = np.array(DataStructs.BulkTanimotoSimilarity(test_mon1_fp, train_mon2_fps))
            mon2_flipped = np.array(DataStructs.BulkTanimotoSimilarity(test_mon2_fp, train_mon1_fps))
            ...
            # Solvent similarity
            solv_similarity = np.array(DataStructs.BulkTanimotoSimilarity(test_solv_fp, train_solv_fps))
            ...
            if np.isnan(best_similarity):
                ...
                filtered_best_positions = np.where(np.abs(combined_similarity - best_similarity) < tol)[0].tolist()
                ...
                if has_nan_test:
                    ...
                    for pos in best_positions:
                        try:
                            ...
                            # Convert to numeric and handle NaN
                            train_features_numeric = pd.to_numeric(train_features, errors='coerce').astype(float)
                            ...
                            feature_dist = np.linalg.norm(test_features_numeric - train_features_numeric)
...
def compute_naive_baseline_predictions_with_similarity(df_test, df_train, y_train, feature_cols=None, fp_dict=None):
    """
...
def plot_confusion_matrix_and_confidence(y_true, y_pred, confidence_scores, correct_mask, output_dir, suffix='', confidence_threshold=0.7, normalize=False):
    ...
    if confidence_threshold is not None and confidence_threshold > 0:
        ...
        print(f"Generating {norm_label}combined plot{' (' + suffix + ')' if suffix else ''} (threshold: {confidence_threshold:.3f})...")
    ...
    disp_filtered = ConfusionMatrixDisplay(
        confusion_matrix=cm_filtered_plot,
        display_labels=[get_class_label(i) for i in range(3)]
    )
...
def plot_baseline_comparison(y_true, y_pred_model, baseline_pred, output_dir, suffix=''):
    ...
    baseline_precision_macro = precision_score(y_true, baseline_pred, average='macro', zero_division=0)
    ...
    print("-" * 65)
    print(f"{'Accuracy':<20} {model_acc:<15.4f} {baseline_acc:<15.4f} {model_acc-baseline_acc:+<15.4f}")
    print(f"{'Precision (weighted)':<20} {model_precision:<15.4f} {baseline_precision:<15.4f} {model_precision-baseline_precision:+<15.4f}")
    print(f"{'Recall (weighted)':<20} {model_recall:<15.4f} {baseline_recall:<15.4f} {model_recall-baseline_recall:+<15.4f}")
    print(f"{'F1-score (weighted)':<20} {model_f1:<15.4f} {baseline_f1:<15.4f} {model_f1-baseline_f1:+<15.4f}")
    
    # Print macro-averaged statistics
    print("\nOverall Metrics (Macro):")
    print(f"{'Metric':<20} {'Model':<15} {'Baseline':<15} {'Difference':<15}")
    print("-" * 65)
    print(f"{'Accuracy (macro)':<20} {model_acc_macro:<15.4f} {baseline_acc_macro:<15.4f} {model_acc_macro-baseline_acc_macro:+<15.4f}")
    print(f"{'Precision (macro)':<20} {model_precision_macro:<15.4f} {baseline_precision_macro:<15.4f} {model_precision_macro-baseline_precision_macro:+<15.4f}")
    print(f"{'Recall (macro)':<20} {model_recall_macro:<15.4f} {baseline_recall_macro:<15.4f} {model_recall_macro-baseline_recall_macro:+<15.4f}")
    print(f"{'F1-score (macro)':<20} {model_f1_macro:<15.4f} {baseline_f1_macro:<15.4f} {model_f1_macro-baseline_f1_macro:+<15.4f}")
    ...
    for cls in range(3):
        ...
        print(f"{class_names[cls]:<10} {model_precision_per_class[cls]:<15.4f} {baseline_precision_per_class[cls]:<15.4f} {model_precision_per_class[cls]-baseline_precision_per_class[cls]:+<15.4f}")
    ...
    for cls in range(3):
        ...
        print(f"{class_names[cls]:<10} {model_recall_per_class[cls]:<15.4f} {baseline_recall_per_class[cls]:<15.4f} {model_recall_per_class[cls]-baseline_recall_per_class[cls]:+<15.4f}")
    ...
    for cls in range(3):
        ...
        if model_precision_per_class[cls] + model_recall_per_class[cls] > 0:
            model_f1_cls = 2 * (model_precision_per_class[cls] * model_recall_per_class[cls]) / (model_precision_per_class[cls] + model_recall_per_class[cls])
        ...
        if baseline_precision_per_class[cls] + baseline_recall_per_class[cls] > 0:
            baseline_f1_cls = 2 * (baseline_precision_per_class[cls] * baseline_recall_per_class[cls]) / (baseline_precision_per_class[cls] + baseline_recall_per_class[cls])
            baseline_f1_per_class.append(baseline_f1_cls)
        else:
            baseline_f1_per_class.append(0.0)
        
        print(f"{class_names[cls]:<10} {model_f1_per_class[cls]:<15.4f} {baseline_f1_per_class[cls]:<15.4f} {model_f1_per_class[cls]-baseline_f1_per_class[cls]:+<15.4f}")
    ...
    for cls in range(3):
        ...
        print(f"{class_names[cls]:<10} {model_acc_per_class[cls]:<15.4f} {baseline_acc_per_class[cls]:<15.4f} {model_acc_per_class[cls]-baseline_acc_per_class[cls]:+<15.4f}")
    ...
    disp_model = ConfusionMatrixDisplay(
        confusion_matrix=cm_model,
        display_labels=class_names
    )
    im_model = disp_model.plot(cmap=CONFUSION_MATRIX_CONFIG['cmap'], ax=ax_cm_model, 
              values_format=CONFUSION_MATRIX_CONFIG['values_format'],
    ...
    im_baseline = disp_baseline.plot(cmap=CONFUSION_MATRIX_CONFIG['cmap'], ax=ax_cm_baseline, 
              values_format=CONFUSION_MATRIX_CONFIG['values_format'],
    ...
    im_diff = disp_diff.plot(cmap='RdBu_r', ax=ax_cm_diff, 
              values_format='.0f',
    ...
    # Plot 1: Precision
    ax1 = axes_metrics[0]
    bars1 = ax1.bar(x - width/2, model_precision_per_class, width, 
                   label='Model', color=COMPARISON_COLORS.get('correct', '#009688'), 
                   alpha=0.7)
    bars2 = ax1.bar(x + width/2, baseline_precision_per_class, width, 
                   label='Naive Baseline', color=COMPARISON_COLORS.get('incorrect', '#e91e63'), 
                   alpha=0.7)
    ...
    # Plot 2: Recall
    ax2 = axes_metrics[1]
    bars3 = ax2.bar(x - width/2, model_recall_per_class, width, 
                   label='Model', color=COMPARISON_COLORS.get('correct', '#009688'), 
                   alpha=0.7)
    bars4 = ax2.bar(x + width/2, baseline_recall_per_class, width, 
                   label='Naive Baseline', color=COMPARISON_COLORS.get('incorrect', '#e91e63'), 
                   alpha=0.7)
    ...
    # Plot 3: F1-score
    ax3 = axes_metrics[2]
    bars5 = ax3.bar(x - width/2, model_f1_per_class, width, 
                   label='Model', color=COMPARISON_COLORS.get('correct', '#009688'), 
                   alpha=0.7)
    bars6 = ax3.bar(x + width/2, baseline_f1_per_class, width, 
                   label='Naive Baseline', color=COMPARISON_COLORS.get('incorrect', '#e91e63'), 
                   alpha=0.7)
    ...
    # Save as PNG
    filename_png = f'baseline_comparison{("_" + suffix.lower().replace(" ", "_")) if suffix else ""}.png'
    path_png = os.path.join(output_dir, filename_png)
    plt.savefig(path_png, dpi=300, bbox_inches='tight')
    print(f"  ✓ Saved PNG to {path_png}")
    
    # Save as PDF
    filename_pdf = f'baseline_comparison{("_" + suffix.lower().replace(" ", "_")) if suffix else ""}.pdf'
    path_pdf = os.path.join(output_dir, filename_pdf)
    plt.savefig(path_pdf, bbox_inches='tight')
    print(f"  ✓ Saved PDF to {path_pdf}")
    ...
    bars_acc1 = ax_acc.bar(x - width/2, model_acc_per_class, width, 
                   label='Model', color=COMPARISON_COLORS.get('correct', '#009688'), 
                   alpha=0.7)
    bars_acc2 = ax_acc.bar(x + width/2, baseline_acc_per_class, width, 
                   label='Naive Baseline', color=COMPARISON_COLORS.get('incorrect', '#e91e63'), 
                   alpha=0.7)
...
def create_latex_performance_table(predictor, output_dir, n_folds=5,
    ...
    try:
        ...
        with open(latex_file, 'w') as f:
            ...
            f.write("\\centering\n")
            f.write("\\caption{Overall performance of the voting model (XGBoost + Lookup). "
                    "Error bars are standard deviations from "
                    f"{n_folds}-fold group-based cross-validation.}}\n")
            f.write("\\begin{tabular}{lccc}\n")
            f.write("\\toprule\n")
            f.write("\\textbf{Metric} & \\textbf{Train} & \\textbf{Test} & \\textbf{Filtered Test} \\\\\n")
            f.write("\\midrule\n")
            f.write(f"Samples & {len(df_train)} & {len(df_test_voting)} & {len(df_test_filtered)} \\\\[3pt]\n")

            f.write(f"Accuracy (macro) & {fmt(train_m, 'accuracy_macro')} & {fmt(test_m, 'accuracy_macro')} & {fmt(filtered_m, 'accuracy_macro')} \\\\\n")
            f.write(f"Accuracy (weighted) & {fmt(train_m, 'accuracy')} & {fmt(test_m, 'accuracy')} & {fmt(filtered_m, 'accuracy')} \\\\[3pt]\n")

            f.write(f"Precision (macro) & {fmt(train_m, 'precision_macro')} & {fmt(test_m, 'precision_macro')} & {fmt(filtered_m, 'precision_macro')} \\\\\n")
            f.write(f"Precision (weighted) & {fmt(train_m, 'precision_weighted')} & {fmt(test_m, 'precision_weighted')} & {fmt(filtered_m, 'precision_weighted')} \\\\\n")
...
def create_latex_per_class_table(predictor, output_dir, confidence_threshold=0.7):
    ...
    try:
        ...
        for name, yt, yp in datasets:
            per_class_prec[name] = precision_score(yt, yp, labels=[0, 1, 2], average=None, zero_division=0)
            per_class_acc[name] = recall_score(yt, yp, labels=[0, 1, 2], average=None, zero_division=0)
        ...
        with open(latex_file, 'w') as f:
            f.write("\\begin{table}[h!]\n")
            f.write("\\centering\n")
            f.write("\\caption{Per-class accuracy and precision of the voting model "
                    "(XGBoost + Lookup).}\n")
            f.write("\\begin{tabular}{lccc}\n")
            f.write("\\toprule\n")
            f.write("\\textbf{Class} & \\textbf{Train} & \\textbf{Test} & \\textbf{Filtered Test} \\\\\n")
...
def generate_plots_for_dataset(df, predictor, args, suffix=''):
    ...
    if generate_all or args.combined:
        plot_confusion_matrix_and_confidence(
            y_true, y_pred, confidence, correct_mask, args.output_dir, suffix,
            confidence_threshold=args.confidence_threshold, normalize=False,
        )
        plot_confusion_matrix_and_confidence(
            y_true, y_pred, confidence, correct_mask, args.output_dir, suffix,
            confidence_threshold=args.confidence_threshold, normalize=True,
        )
