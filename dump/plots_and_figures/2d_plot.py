...
def sort_and_combine_fingerprints(fingerprint_1, fingerprint_2, fingerprint_solvent):
    ...
    combined_fingerprint = (
        np.array(fingerprint_1)
        + np.array(fingerprint_2)
        + np.array(fingerprint_solvent)
    )
...
for entry in data:
    ...
    combined_fingerprint = sort_and_combine_fingerprints(
        fingerprint_1, fingerprint_2, fingerprint_solvent
    )
...
patches = [
    ...
    for color, label in zip(["blue", "green", "purple", "orange"], unique_labels)
]
