...
try:
    ...
    print("Warning: copolextractor.utils not available. IUPAC names will use monomer names from reactions.")
...
def clean_reaction_data(reaction_data: Dict[str, Any], 
                        polytype_pca_map: Dict[str, Dict],
                        method_pca_map: Dict[str, Dict]) -> Optional[Dict[str, Any]]:
    """
...
def copy_monomer_files(reactions_by_doi: Dict[str, List], 
...
def process_reactions(input_dir: Path, output_dir: Path, 
        ...
        try:
            ...
            for field in ['monomer1_smiles', 'monomer2_smiles', 'monomer1_s', 'monomer2_s']:
                if field in data:
                    ...
                    elif isinstance(value, str) and value.strip().lower() in ('none', 'na', 'nan', 'null', ''):
